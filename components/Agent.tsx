...
const Agent = ({userName, userId, type, interviewId, questions} : AgentProps) => {
    ...
    useEffect(() => {
        if(callStatus === CallStatus.CONNECTING) {
            const toastStatus = toast.loading('Connecting to AI Interviewer');
            setToastId(toastStatus);
            ...
            <div className="w-full flex justify-center">
                {callStatus !== 'ACTIVE' ? (
                    <button className="relative btn-call" onClick={handleCall}>
                        <span
                          className={cn(
                              'absolute animate-ping rounded-full opacity-75', callStatus !== 'CONNECTING' && 'hidden'
                          )}
                      />
                        <span>
